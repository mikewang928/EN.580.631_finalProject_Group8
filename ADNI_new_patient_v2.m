...
z_Normal(end+1,:) = z_new_patient_normal_corrected;
z_ADNI(end+1,:) = z_new_patient_ADNI_corrected;
...
h_y_label_normal(end+1) = {"new_patient"};
h_y_label_ADNI = T(ind_ADNI,:).x_Subject_ID_;
h_y_label_ADNI(end+1) = {"new_patient"};
h_x_label = Normal_table.Properties.VariableNames;

%%
hm_normal = heatmap(h_x_label,h_y_label_normal,z_Normal);
hm_normal.Title =  'new patient heatmap withRespectTo Normal dataset [CORRECTED]';
cmapeditor

%%
hm_ADNI= heatmap(h_x_label,h_y_label_ADNI,z_ADNI);
hm_ADNI.Title =  'new patient heatmap withRespectTo ADNI dataset [CORRECTED]';
cmapeditor
...
elseif prediction_corrected == 1
    disp("+ the new patient has ADNI based on the corrected segmented volume data")
end
